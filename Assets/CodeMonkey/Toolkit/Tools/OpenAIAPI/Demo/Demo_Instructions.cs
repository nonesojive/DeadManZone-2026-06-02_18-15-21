...
namespace CodeMonkey.Toolkit.TOpenAIAPI.Demo {
    ...
    public class Demo_Instructions : MonoBehaviour {


        [SerializeField] private TMP_InputField instructionsInputField;
        ...
        private void Awake() {
            sendPromptButton.onClick.AddListener(() => {
                Debug.Log("OpenAI Instructions: " + instructionsInputField.text + "; Prompt: " + promptInputField.text);
                outputTextMesh.text = "Asking ChatGPT...";
                Demo.Instance.ShowLoading();
                OpenAIAPI.AskChatGPT(
                    new AskChatGPTData {
                        model = "gpt-5",
                        instructions = instructionsInputField.text,

...
namespace CodeMonkey.Toolkit.TWebRequests {
    ...
    public static class WebRequests {
        ...
        public static void PostJson(string url, string jsonData, Dictionary<string, string> headerDictionary, Action<string> onError, Action<string> onSuccess) {
            Init();
            webRequestsMonoBehaviour.StartCoroutine(GetCoroutinePostJson(url, jsonData, headerDictionary, onError, onSuccess));
        }
        ...
        private static IEnumerator GetCoroutinePostJson(string url, string jsonData, Dictionary<string, string> headerDictionary, Action<string> onError, Action<string> onSuccess) {
            using (UnityWebRequest unityWebRequest = new UnityWebRequest(url, "POST")) {
                byte[] bodyRaw = Encoding.UTF8.GetBytes(jsonData);
                unityWebRequest.uploadHandler = (UploadHandler)new UploadHandlerRaw(bodyRaw);
                unityWebRequest.downloadHandler = (DownloadHandler)new DownloadHandlerBuffer();
                foreach (string key in headerDictionary.Keys) {
                    unityWebRequest.SetRequestHeader(key, headerDictionary[key]);
                }
                unityWebRequest.SetRequestHeader("Content-Type", "application/json");

...
#if ENABLE_INPUT_SYSTEM
            float mouseX = Mouse.current.delta.value.x;
            float mouseY = Mouse.current.delta.value.y;
#else
...
#if ENABLE_INPUT_SYSTEM
            bool isSpaceKeyPressed = Keyboard.current.spaceKey.isPressed;
#else
            bool isSpaceKeyPressed = Input.GetKey(KeyCode.Space);
#endif
            if (characterController.isGrounded && isSpaceKeyPressed) {

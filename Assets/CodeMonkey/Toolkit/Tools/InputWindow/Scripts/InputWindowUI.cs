    ...
    public class InputWindowUI : MonoBehaviour {
        ...
        private void Update() {
            if (listenToKeyInputs) {
                ...
                bool isEnterKeyDown = Keyboard.current.enterKey.wasPressedThisFrame;
                bool isKeypadEnterKeyDown = Keyboard.current.numpadEnterKey.wasPressedThisFrame;
#else
                bool isEnterKeyDown = Input.GetKeyDown(KeyCode.Return);
                bool isKeypadEnterKeyDown = Input.GetKeyDown(KeyCode.KeypadEnter);
#endif
                if (isEnterKeyDown || isKeypadEnterKeyDown) {
                    okClickAction?.Invoke();
                }
#if ENABLE_INPUT_SYSTEM
                bool isEscapeKeyDown = Keyboard.current.escapeKey.wasPressedThisFrame;
#else
                bool isEscapeKeyDown = Input.GetKeyDown(KeyCode.Escape);
#endif
                if (isEscapeKeyDown) {
                    cancelClickAction?.Invoke();
                }

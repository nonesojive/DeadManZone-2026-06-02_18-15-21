...
## Interactive tactics window (2026-07-11)

The 3D demo's command pauses are now the REAL interactive tactics flow — the reference implementation for the main-build port. The driver no longer pre-runs the whole fight: it runs interleaved (`Continue(pending)` → replay segment → window at every `AwaitingCommand` → repeat), and the player's PhaseCommands feed the same `TickCombatRun.Continue` the live game uses. Core untouched (0 Core diffs), 2D path untouched.

### Architecture (four Presentation files, no god-class)
- **`CombatTacticOrdersDraft`** (plain C#, EditMode-testable) — the in-progress order sheet: selected doctrine + queued abilities. Pure state + budget arithmetic; every accept/reject verdict comes from Core's `TacticPauseValidator` (`ValidatePause`/`GetTotalPauseCost`) — the class never re-implements a rule. `BuildCommands()` emits exactly the live flow's shape: `SetTactic` first (`SourcePieceId = "player_tactic"`, mirroring `TacticPausePanel`), then `UseAbility` commands with `TargetCell`.
- **`CombatTacticTargetPicker`** (MonoBehaviour) — battlefield target selection for targeted abilities. Mouse ray ∩ ground plane y=0 (arena ships no colliders) → `CombatGridMapper.TryWorldToCoord` → caller predicate → confirm. `TryPickWorldPoint(worldPoint)` is the single accept path — real ground clicks and programmatic verification go through the same code. Amber pulse hover reticle (valid) / dark red (invalid), persistent amber pending markers on queued targets (DMZ/CombatRingFill quads, max-contrast targeting register per bible §6 overlay exception).
- **`CombatTacticOrdersWindow`** (MonoBehaviour) — runtime-built grimdark UI, canvas sort 450 (HUD 400 < window < banner 500), battlefield dim behind. Header band + "TACTICAL PAUSE — ORDERS" + trigger-aware subtitle; live `AUTHORITY n / total (COMMITTED k)` line; doctrine column (current selection leather-highlighted, locked tactics dimmed + non-interactable); per-unit ability buttons (name · source piece · cost, dimmed "— QUEUED" once queued); queued-orders list with per-command CANCEL; rejection feedback = panel shake + red validator reason verbatim; RESUME (+ Space/Esc) gated on `Draft.Validate`. During target pick the window drops to 10% alpha and stops blocking raycasts; a SELECT TARGET hint band explains LMB/RMB.
- **`Combat3DDemoDriver`** — restructured to the interleaved loop above. Serialized `interactivePauses` (default ON); OFF = exactly the old auto path (no commands, TACTICAL PAUSE beat between segments) for headless verification. `BuildPauseContext()` snapshots the run the same way `RunOrchestrator.GetCombatPauseContext` does (CommandProcessor.GetAvailableCommands, hasCommandPiece, faction startingTactics, authority). Determinism: the sim consumes the player's actual commands exactly like the real game; holding at a pause is pure presentation.

### Content gotcha + demo-only ability overrides
The current IronMarch content pass ships **zero ability-granting pieces** (`grantedAbility: 0` on every piece asset; `grenade_thrower` no longer exists — `DemoPieceFactory` assigns GrenadeLob/ShieldAllies/CannonBlast to neutral pieces, but the IronMarch pass overwrote/replaced that set). The demo roster therefore supports `"pieceId:Ability"` entries (default: `armored_transport:ShieldAllies`, `ironclad_mortars:GrenadeLob`) which clone the `PieceDefinitionSO` at runtime (`Instantiate` + set field) — assets untouched, enemy side unaffected. When a content pass ships real ability pieces, drop the suffixes.

### Verification (programmatic, via the real onClick handlers — seed 20260711, authority 8)
- **Interactive**: ShieldAllies queued (8→6 shown live), GrenadeLob → pick mode → `TryPickTargetAtWorldPoint(mapper.ToWorld(enemyAnchor))` accepted at (12,1), off-grid/empty cells rejected by the same path; CANCEL refunded (4→6 remaining), re-add restored; RESUME submitted; at pause 1 switched doctrine to ADVANCE (cost 1). Result: **VICTORY** (first ever on this seed), 54 events incl. `tactic_set:2, grenade_lob:2 (one enemy destroyed at the opening pause), shield_allies:1`, authority 8→3 (2+2+1, matching the validator table), banner fired, timeScale restored.
- **Divergence proof**: identical armies/seed run headlessly with empty command batches → **DEFEAT**, 77 events, authority untouched at 8, zero ability events; live vs baseline logs diverge at event index 0.
- **Non-interactive path** (`interactivePauses=false`, scene toggled + saved + played): DEFEAT, 77 events — event-for-event the no-command baseline; orders window never instantiated; pause beat fired between segments; banner OK. Flag restored to ON and scene saved.
- Menu rebuild (`DeadManZone → Combat3D → Build Combat3D Demo Scene`) includes the wiring — window + picker are runtime `AddComponent`s on the driver, so the rebuild needs no extra objects; fresh driver defaults (roster + interactivePauses) land via `AddComponent`.
- EditMode **365/365** green (358 baseline + 7 new `CombatTacticOrdersDraftTests` covering costs, over-budget/duplicate rejection, cancel refund, locked-tactic rejection, checkpoint-1 switch cost, and the exact BuildCommands shape).
- One UI bug found live and fixed: `VerticalLayoutGroup.childControlHeight` must be **true** or Unity ignores `LayoutElement` heights and rows render at the default 100px rect (queued rows overflowed the footer).

### Main-build porting notes
- **Reusable as-is**: `CombatTacticOrdersDraft` (feed it `CombatPauseContext` fields; it already matches `TacticPausePanel`'s submit shape, so `RunOrchestrator.SubmitCombatCommands(draft.BuildCommands())` just works), `CombatTacticTargetPicker` (needs only a mapper + camera + validity predicate), the window's layout/skin/interaction patterns.
- **Demo-only, replace when porting**: `BuildPauseContext()` (use `RunOrchestrator.GetCombatPauseContext` — already shaped identically), the `IsValidAbilityTarget` predicate reading `EnemyCombatantsForTests` (surface enemy anchors through `CombatPauseContext` instead; the Core executor honors any occupied cell of a live enemy — the demo predicate is anchor-only, i.e. strictly conservative), the roster ability overrides, and the driver's interleaved loop (the run scene already has its own advance loop — only the "open window at AwaitingCommand, feed commands to Continue" seam ports).
- **Draft persistence**: the live flow saves pause drafts (`SavePauseDraft`/`PendingSelectedAbilities`) — the demo window doesn't persist; wire `Draft` state into that path when porting.

### Spec compliance (50-combat-arena-spec + bible)
Muted grimdark kit throughout (`CombatGrimdarkSkin` bands/leather/bone — no new colors beyond the amber targeting register, which bible §6 reserves for mechanical overlays); no full-screen interrupts — the window is the pause, combat never plays under it; saturation budget respected (dim at 55% black, panel smoky flat). Sort order 450 keeps HUD readable above and banner supreme.

### Deferred
- CannonBlast targeting is wired (`RequiresTarget` includes it) but no demo piece grants it — lights up with content.

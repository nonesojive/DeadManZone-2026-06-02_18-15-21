...
- `ApplyGas(int segment)` — `int damage = GasDamageSystem.GetDamage(combatant.Position, GlobalTick - CombatPacingConfig.GasStartTick, _layout);` and `_log.Append(segment, GlobalTick, "gas", "gas_damage", combatant.InstanceId, damage);` death log via `LogDestroyed(segment, ...)`. **Bug fix:** the old code had actionType/targetId swapped (`"gas", combatant.InstanceId, "gas_damage"`) — the corrected order above puts `"gas_damage"` in ActionType and the victim in TargetId, matching every other damage event.
